function Camera(gl, d, modelUp) {
  ...
  var diagonal = Math.sqrt(
    Math.pow(d.max[0] - d.min[0], 2) +
      Math.pow(d.max[1] - d.min[1], 2) +
      Math.pow(d.max[2] - d.min[2], 2)
  );
  ...
  this.dolly = function(view, delta) {
    ...
    var newview = new Matrix4().translate(
      -view.elements[2] * delta,
      -view.elements[6] * delta,
      -view.elements[10] * delta
    );
    var newEye = newview.multiplyVector4(
      new Vector4([eye[0], eye[1], eye[2], 1])
    ).elements;
    //console.log(eye);
    eye = newEye;

    var newAt = newview.multiplyVector4(new Vector4([at[0], at[1], at[2], 1]))
      .elements;
    //console.log(eye);
    at = newAt;
  };
  this.truck = function(view, delta) {
    var newview = new Matrix4().translate(
      -view.elements[0] * delta,
      -view.elements[4] * delta,
      -view.elements[8] * delta
    );
  ...
  this.pedestal = function(view, delta) {
    var newview = new Matrix4().translate(
      -view.elements[1] * delta,
      -view.elements[5] * delta,
      -view.elements[9] * delta
    );
    //var newEye = new Vector3(view.mu
    var newEye = newview.multiplyVector4(
      new Vector4([eye[0], eye[1], eye[2], 1])
  ...
  this.getRotatedCameraPosition = function(angle) {
    var m = new Matrix4()
      .setTranslate(at[0], at[1], at[2])
      .rotate(angle, up[0], up[1], up[2])
      .translate(-at[0], -at[1], -at[2]);
    var e = m.multiplyVector4(new Vector4([eye[0], eye[1], eye[2], 1]))
      .elements;
    return [e[0], e[1], e[2]];
  };
